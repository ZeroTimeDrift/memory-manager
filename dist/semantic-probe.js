...
function buildTestCases() {
    ...
    return cases.filter(c => {
        if (exists(c.expectedPath))
            return true;
        if (c.expectedAlts?.some(a => exists(a)))
            return true;
...
function searchVector(query, limit = 10) {
    try {
        ...
        const embResult = child_process.execSync(`node -e "
        const fs = require('fs');
...
function runProbe(bm25Only = false) {
    ...
    for (const tc of testCases) {
        ...
        if (!categories[tc.category])
            categories[tc.category] = { total: 0, found: 0 };
        categories[tc.category].total++;
        if (found)
            categories[tc.category].found++;
        // Track by lexical overlap
        if (!overlapAnalysis[tc.lexicalOverlap])
            overlapAnalysis[tc.lexicalOverlap] = { total: 0, found: 0 };
        overlapAnalysis[tc.lexicalOverlap].total++;
        if (found)
            overlapAnalysis[tc.lexicalOverlap].found++;
        const result = {
            testCase: tc,
            bm25Found: found,
            bm25Rank: rank,
