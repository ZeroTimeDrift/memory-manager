...
function scoreSession(filePath) {
    ...
    for (const line of lines) {
        try {
            ...
            if (entry.type === 'message') {
                ...
                // Content signals
                if (/decision|decided|agree|confirm/i.test(text))
                    hasDecisions = true;
                if (/memory|recall|search|capture|survive/i.test(text))
                    hasMemoryContent = true;
                if (/moongate|widget|dashboard|memeramper/i.test(text))
                    hasMoonGateContent = true;
                if (/identity|soul|conscious|prometheus|anchor/i.test(text))
            ...
            if (entry.type === 'message' && entry.message?.role === 'assistant') {
                ...
                if (Array.isArray(content)) {
                    toolCallCount += content.filter((c) => c.type === 'tool_use' || c.type === 'tool_call').length;
                }
            }
        }
        catch {
    ...
    if (hevarMessageCount >= 5)
        score += 30; // Deep conversation
    else if (hevarMessageCount >= 2)
        score += 20; // Brief exchange
    else if (hevarMessageCount >= 1)
        score += 10; // Single message
...
function rankSessions(limit = 20) {
...
// ─── Run Recall Tests ───────────────────────────────────────────────────────
function runRecallTests(tests) {
    const details = [];
    let passed = 0;
    for (const test of tests) {
        try {
            const result = child_process.execSync(`clawdbot memory search "${test.query.replace(/"/g, '\\"')}" --json --max-results 5 2>/dev/null`, { encoding: 'utf-8', timeout: 30000, maxBuffer: 10 * 1024 * 1024 });
    ...
    const weights = { critical: 3, high: 2, medium: 1.5, low: 1 };
...
// ─── Run Priority Tests ─────────────────────────────────────────────────────
function runPriorityTests(tests) {
    const details = [];
    let passed = 0;
...
`);
        const output = child_process.execSync(`cd ${WORKSPACE}/skills/memory-manager && npx ts-node ${tmpScript}`, { encoding: 'utf-8', timeout: 30000, maxBuffer: 10 * 1024 * 1024 }).trim();
        // Clean up temp file
        try {
...
async function main() {
    ...
    const scores = [];
    if (result.recall.tests > 0)
        scores.push({ w: 0.5, s: result.recall.score });
    if (result.priority.tests > 0)
        scores.push({ w: 0.25, s: result.priority.score });
    if (result.sessions.total > 0)
        scores.push({ w: 0.25, s: result.sessions.score });
    ...
    saveResults(result);
    try {
        const history = JSON.parse(fs.readFileSync(RESULTS_PATH, 'utf-8'));
        showTrend(history);
    }
    catch { }
    console.log('');
