...
exports.graphExpand = graphExpand;
exports.parseTemporalRef = parseTemporalRef;
exports.reciprocalRankFusion = reciprocalRankFusion;
exports.extractQueryEntities = extractQueryEntities;
exports.conceptIndexSearch = conceptIndexSearch;
...
function parseTemporalRef(query, now) {
    const ref = now || new Date();
    const lower = query.toLowerCase().trim();
    // ISO date
    const isoMatch = lower.match(/(\d{4})-(\d{2})-(\d{2})/);
    if (isoMatch)
        return resolveTemporalFiles({ type: 'exact', dates: [isoMatch[0]], confidence: 1.0, matchedText: isoMatch[0], files: [] });
    // Month + day
    const mdPatterns = [
    ...
    if (/\btoday\b/.test(lower))
        return resolveTemporalFiles({ type: 'relative', dates: [formatDate(ref)], confidence: 0.9, matchedText: 'today', files: [] });
    if (/\byesterday\b/.test(lower))
        return resolveTemporalFiles({ type: 'relative', dates: [formatDate(addDays(ref, -1))], confidence: 0.9, matchedText: 'yesterday', files: [] });
    const daysAgo = lower.match(/\b(\d+)\s+days?\s+ago\b/);
    if (daysAgo)
        return resolveTemporalFiles({ type: 'relative', dates: [formatDate(addDays(ref, -parseInt(daysAgo[1])))], confidence: 0.85, matchedText: daysAgo[0], files: [] });
    // Day of week
    ...
    if (/\bthis\s+week\b/.test(lower)) {
        ...
        if (fs.existsSync(weeklyPath) && !info.files.includes(weeklyPath))
            info.files.unshift(weeklyPath);
        return info;
    }
...
function resolveTemporalFiles(info) {
    const files = [];
 ...
 */
function graphExpand(entities, originalQuery, maxExpansions = 2) {
    ...
    for (const entity of entities) {
        ...
        for (const related of entry.related) {
            ...
            if (entities.includes(related))
                continue;
            if (queryLower.includes(related.toLowerCase()))
                continue;
            // Skip noisy edges (file paths, raw text fragments)
            if (related.includes('/') || related.includes(':') || related.includes('.md'))
                continue;
            if (related.length < 3 || related.length > 40)
                continue;
...
// ─── Temporal Search ────────────────────────────────────────────────────────
function temporalFileSearch(temporal, query, limit = 10) {
    ...
    const contentTokens = cleanQuery.match(/[A-Za-z0-9_]+/g)?.filter(t => t.length > 2 && !['what', 'did', 'the', 'how', 'was', 'were', 'are', 'has', 'had', 'have', 'been', 'being', 'about', 'from', 'that', 'this', 'with', 'for'].includes(t.toLowerCase())) ?? [];
    for (const file of temporal.files) {
        const relPath = file.replace('/root/clawd/', '');
        try {
            const chunks = db.prepare(`SELECT path, text FROM chunks WHERE path = ? AND source='memory' ORDER BY start_line ASC`).all(relPath);
 ...
 */
function conceptIndexSearch(entities, limit = 10) {
    if (entities.length === 0)
        return [];
    let index;
    try {
    ...
    for (const entity of entities) {
        ...
        if (!entry || !entry.files)
            continue;
        for (const [filePath, info] of Object.entries(entry.files)) {
    ...
    for (let i = 0; i < sorted.length; i++) {
        ...
        try {
            ...
            const chunk = db.prepare(`SELECT text FROM chunks WHERE path = ? AND source='memory' ORDER BY start_line ASC LIMIT 1`).get(filePath);
            if (chunk?.text) {
                snippet = chunk.text.substring(0, 300);
            }
        }
 ...
 */
function reciprocalRankFusion(resultSets, k = 60, limit = 10) {
...
function smartSearch(query, limit = 10, referenceDate) {
    ...
    for (const variant of allExpanded) {
        ...
        const tagged = varResults.map(r => ({ ...r, source: 'expansion' }));
...
function isResultMatch(result, test) {
...
function findRank(results, test) {
    for (let i = 0; i < results.length; i++) {
        if (isResultMatch(results[i], test))
            return i + 1;
    }
    return null;
}
function runBenchmark(referenceDate) {
    ...
    for (const test of BENCHMARK_TESTS) {
        ...
        if (!byCategory[test.category])
            byCategory[test.category] = { smart: 0, baseline: 0, total: 0 };
        ...
        if (smart.temporal)
            console.log(`       Temporal: ${smart.temporal.type} → ${smart.temporal.files.map(f => path.basename(f)).join(', ')}`);
        if (smart.conceptMatches.length > 0)
            console.log(`       Concepts: ${smart.conceptMatches.join(', ')}`);
        if (smart.expanded.length > 0)
            console.log(`       Expanded: ${smart.expanded.length} variants`);
        console.log();
    }
    // Summary
    console.log('───────────────────────────────────────────────────────');
