 ...
 */
function extractKeyItems(content, filename) {
    const items = [];
    ...
    for (const line of lines) {
        ...
        if (trimmed === '---')
            continue;
        if (/^(date|day|tags|mood):/.test(trimmed))
            continue;
        // Skip empty lines
        if (!trimmed)
            continue;
        ...
        if (/^\d{2}:\d{2}/.test(trimmed))
            continue; // Pure timestamps
        if (/heartbeat|HEARTBEAT_OK/i.test(trimmed))
            continue;
        if (/^-\s*\[[\sx]\]/.test(trimmed))
            continue; // Checkboxes (tasks tracked elsewhere)
    }
    return items;
}
 ...
 */
function ensureWeeklyDigest(date, items) {
    if (items.length === 0)
        return false;
    // Calculate ISO week
...
// ─── Main Pruning Logic ────────────────────────────────────────────────────
function scanDailyFiles(maxAgeDays) {
...
function pruneDaily(maxAgeDays, execute, verbose) {
