...
function analyzeMemoryState() {
    ...
    function scanRecent(dir, prefix = '') {
...
function getSessionHistory(manifest) {
    return manifest.sessionHistory || [];
}
function getConsolidationStreak(history) {
    let streak = 0;
...
function getLastTaskOfCategory(history, category) {
    for (let i = history.length - 1; i >= 0; i--) {
        if (history[i].taskCategory === category)
            return history[i];
    }
    return null;
}
...
// ─── Progress Blockers ──────────────────────────────────────────────────────
function detectProgressBlockers() {
    const blockers = [];
 ...
 */
function detectSpecificGap(manifest, memoryState) {
    const gaps = [];
    ...
    if (fs.existsSync(learnLogPath)) {
        try {
            ...
            if (latest && latest.patterns) {
                const highPatterns = latest.patterns.filter((p) => p.severity === 'high' || p.severity === 'medium');
                if (highPatterns.length > 0) {
                    gaps.push({
                        task: `Fix ${highPatterns.length} retrieval gap(s) from learning loop`,
                        context: `retrieval-learn.ts found: ${highPatterns.map((p) => p.type + ' in ' + (p.file || 'unknown')).join('; ')}. Run retrieval-learn.ts, then manually fix any chunks or vocab gaps it can't auto-fix.`,
                        priority: 2,
...
function generateIntelligentTask(manifest) {
    ...
    for (const strategic of STRATEGIC_BACKLOG) {
        ...
        if (completedStrategic.has(taskSig))
            continue;
        ...
        if (isStrongMatch) {
            ...
            if (!manifest.completedStrategicTasks)
                manifest.completedStrategicTasks = [];
        ...
        if (strategic.condition && !strategic.condition(manifest, memoryState))
            continue;
        ...
        if (recentCategories.includes(strategic.category))
            continue;
    ...
    try {
        ...
        const recent = (Array.isArray(log) ? log : []).slice(-3);
        recentRunsAllClean = recent.length >= 3 && recent.every((r) => (r.summary || '').includes('GOOD_ENOUGH') ||
            (r.patterns || []).every((p) => p.severity === 'low') ||
            (r.patterns || []).length === 0);
    }
    catch { }
 ...
 */
function recordSession(taskCategory, taskName, outcome) {
    const manifest = JSON.parse(fs.readFileSync(MANIFEST_PATH, 'utf-8'));
    if (!manifest.sessionHistory)
