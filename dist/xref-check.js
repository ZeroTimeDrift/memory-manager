...
function getFileHeaders(absPath) {
    if (headerCache.has(absPath))
        return headerCache.get(absPath);
...
function extractRefs(filePath, content, sectionAnchors) {
    const refs = [];
...
function resolveRef(sourceFile, lineNum, rawTarget, refType) {
...
// ─── Report Generation ─────────────────────────────────────────────────────
function generateReport(verbose) {
    ...
    const isHistoricalRef = (ref) => (ref.sourceFile.includes('daily/') || ref.sourceFile.includes('sessions/'));
...
if (jsonOutput) {
    const out = {
        ...
        healthScore: Math.round((report.totalRefs > 0 ? (1 - report.brokenRefs.length / report.totalRefs) * 100 : 100) * 0.7 +
