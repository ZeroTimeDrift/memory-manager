...
function sqlite(query) {
    try {
        return (0, child_process_1.execSync)(`sqlite3 "${DB_PATH}" "${query.replace(/"/g, '\\"')}"`, { encoding: 'utf-8' }).trim();
    }
    catch {
        return '';
    }
...
function getChunksByFile() {
    ...
    for (const line of raw.split('\n').filter(Boolean)) {
        ...
        const row = { path: p, start_line: +s, end_line: +e, text_len: +l };
        if (!map.has(p))
            map.set(p, []);
        map.get(p).push(row);
    }
...
function analyzeOverlap(chunks) {
    if (chunks.length < 2)
        return 'good';
    let totalOverlap = 0;
    let pairs = 0;
