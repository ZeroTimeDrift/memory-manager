...
exports.parseTemporalQuery = parseTemporalQuery;
exports.temporalSearch = temporalSearch;
exports.stripTemporalWords = stripTemporalWords;
 ...
 */
function parseTemporalQuery(query, referenceDate) {
    const now = referenceDate || getNow();
    const lower = query.toLowerCase().trim();
    const noRef = { type: 'none', dates: [], confidence: 0, matchedText: '', files: [] };
...
function getDailyFilesForPrefix(prefix) {
    try {
 ...
 */
function temporalSearch(query, limit = 10, referenceDate) {
    ...
    if (temporal.type !== 'none' && temporal.files.length > 0) {
        ...
        const contentTokens = cleanQuery.match(/[A-Za-z0-9_]+/g)?.filter(t => t.length > 2 && !['what', 'did', 'the', 'how', 'was', 'were', 'are', 'has', 'had', 'have', 'been', 'being'].includes(t.toLowerCase())) ?? [];
        for (const file of temporal.files) {
            const relPath = file.replace('/root/clawd/', '');
            try {
                // Get all chunks from this temporal file
                const allChunks = db.prepare(`SELECT path, source, text FROM chunks WHERE path = ? AND source='memory' ORDER BY start_line ASC`).all(relPath);
    ...
    if (tokens.length > 0) {
        ...
        try {
            const ftsRows = db.prepare(`SELECT path, source, bm25(chunks_fts) AS rank, text
         FROM chunks_fts
...
function runBenchmark(referenceDate) {
    ...
    for (const test of TEMPORAL_TESTS) {
        ...
        const matchIdx = results.findIndex(r => r.path.includes(test.expectedFile) ||
