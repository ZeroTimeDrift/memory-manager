...
function extractMetadata(frontmatter, key) {
    const match = frontmatter.match(new RegExp(`^${key}:\\s*(.+)$`, 'm'));
    return match ? match[1].trim() : undefined;
}
function extractBulletPoints(content, keywords) {
    const points = [];
    const lines = content.split('\n');
...
function extractFileReferences(content) {
    ...
    const files = [...new Set(matches.filter(file => !file.includes('YYYY') &&
            !file.includes('XX') &&
            file.length > 3))];
    return files;
}
function generateSessionSummary(metadata, keyActions, filesModified, decisions) {
    const parts = [];
    // Focus and outcome
    if (metadata.focus) {
...
function updateDailyNotes(sessionSummary) {
