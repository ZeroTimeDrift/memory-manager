...
exports.contextExpand = contextExpand;
...
function classifyFile(filePath) {
    if (['SOUL.md', 'IDENTITY.md', 'USER.md', 'MEMORY.md', 'TOOLS.md'].includes(filePath))
        return 'core';
    if (filePath.includes('daily/'))
        return 'daily';
    if (filePath.includes('weekly/'))
        return 'weekly';
    if (filePath.includes('topics/'))
        return 'topic';
    if (filePath.includes('people/'))
        return 'people';
    if (filePath.includes('OPERATING'))
        return 'operational';
    if (filePath.includes('core-identity') || filePath.includes('rules-reference'))
        return 'proxy';
 ...
 */
function buildEntityEdges(files) {
    const edges = [];
    let conceptIndex;
    try {
    ...
    for (const [conceptName, concept] of Object.entries(conceptIndex.concepts || {})) {
        if (NOISE_CONCEPTS.has(conceptName))
            continue;
        ...
        if (conceptFiles.length < 2 || conceptFiles.length > 8)
            continue;
 ...
 */
function buildTemporalEdges(files) {
    const edges = [];
 ...
 */
function buildSectionEdges(files) {
    const edges = [];
...
function detectClusters(nodes, edges) {
    ...
    for (const [, members] of clusterMembers) {
        ...
        for (const member of members) {
            if (nodes[member])
                nodes[member].cluster = clusters[clusters.length - 1].id;
        }
    }
 ...
 */
function getNeighbors(filePath, limit = 10) {
    const graph = loadGraph();
    // Fuzzy match: allow partial paths
    let nodePath = filePath;
    if (!graph.nodes[filePath]) {
        const match = Object.keys(graph.nodes).find(n => n.includes(filePath) || n.endsWith(filePath) || path.basename(n, '.md') === filePath);
        if (match)
            nodePath = match;
        else {
    ...
    for (const [target, weight] of Object.entries(node.edges)) {
        ...
        const edge = graph.edges.find(e => (e.source === nodePath && e.target === target) ||
 ...
 */
function contextExpand(query, maxFiles = 8) {
    const graph = loadGraph();
    // Step 1: Get initial results from memory_search via clawdbot CLI
    let initialResults = [];
    try {
        const result = require('child_process').execSync(`clawdbot memory search "${query.replace(/"/g, '\\"')}" --json --max-results 5 2>/dev/null`, { encoding: 'utf-8', timeout: 15000 });
        const data = JSON.parse(result);
        initialResults = (data.results || []).map((r) => ({
            path: r.path || '',
            score: r.score || 0,
        }));
    }
 ...
 */
function findIsolated(threshold = 2) {
    const graph = loadGraph();
    ...
    if (graph.clusters.length > 0) {
        ...
        for (const cluster of graph.clusters) {
            ...
            if (cluster.members.length > 5)
                console.log(`        ... +${cluster.members.length - 5} more`);
        }
    }
    // Isolated
