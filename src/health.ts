...
interface Manifest {
  ...
  config: {
    ...
    weightDecayPerDay: number;
    minCoreWeight: number;
  };
  ...
  lastDecayRun: string;
  completedStrategicTasks: string[];
  recentTopics: string[];
}
...
interface HealthReport {
  ...
  weights: {
    avg: number;
    min: { file: string; weight: number };
    max: { file: string; weight: number };
    byType: Record<string, { count: number; avgWeight: number }>;
    distribution: { high: number; medium: number; low: number; stale: number };
  };
  staleness: {
    staleFiles: Array<{ file: string; weight: number; lastAccess: string; daysSince: number }>;
    oldestAccess: { file: string; date: string; daysSince: number };
    newestAccess: { file: string; date: string };
  };
  decay: {
    lastRun: string;
    daysSinceDecay: number;
    filesWithDecay: number;
    filesProtected: number;  // decayRate === 0
  };
  sessions: {
    total: number;
    last7Days: number;
...
function findMemoryFiles(): string[] {
  ...
  function walk(dir: string, prefix: string) {
    ...
    for (const e of entries) {
      ...
      if (e.isDirectory()) {
        if (e.name === 'archive') continue; // Skip archived files
...
function loadBenchmarkHistory(): any[] {
  try {
    if (!fs.existsSync(BENCHMARK_PATH)) return [];
...
function analyze(manifest: Manifest): HealthReport {
  const now = new Date();
  ...
  if (orphans.length > 5) warnings.push(`${orphans.length} orphan files not in manifest`);
  if (missing.length > 0) warnings.push(`${missing.length} manifest entries point to missing files`);
  ...
  for (const [, v] of entries) {
    if (!byType[v.type]) byType[v.type] = { count: 0, totalWeight: 0 };
    byType[v.type].count++;
    byType[v.type].totalWeight += v.weight;
  }
  const byTypeAvg: Record<string, { count: number; avgWeight: number }> = {};
  ...
  for (const w of weights) {
    if (w >= 0.7) distribution.high++;
    else if (w >= 0.3) distribution.medium++;
    else if (w >= 0.1) distribution.low++;
  ...
  const staleFiles: Array<{ file: string; weight: number; lastAccess: string; daysSince: number }> = [];
  ...
  if (daysSinceDecay > 2) warnings.push(`Decay hasn't run in ${daysSinceDecay} days`);
  ...
  for (let i = sessions.length - 1; i >= 0; i--) {
    if (sessions[i].outcome === 'completed') streak++;
    else break;
  }

  // --- Benchmarks ---
  const benchmarks = loadBenchmarkHistory();
  let benchmarkInfo: HealthReport['benchmarks'] = { available: false };
  if (benchmarks.length > 0) {
    ...
    if (benchmarks.length >= 3) {
      const recent3 = benchmarks.slice(-3).map((b: any) => b.recallScore ?? b.score ?? 0);
  ...
  if (avgWeight >= 0.5) score += 10;
  else if (avgWeight >= 0.3) score += 5;
  else score -= 5;
  ...
  if (last7.length >= 5) score += 10;
  else if (last7.length >= 3) score += 5;

  // Decay health
  if (daysSinceDecay <= 1) score += 5;
  else if (daysSinceDecay > 3) score -= 10;

  // Benchmark bonus
  if (benchmarkInfo.available && benchmarkInfo.trend === 'improving') score += 5;
  if (benchmarkInfo.available && benchmarkInfo.trend === 'declining') score -= 5;

  // Clamp
  score = Math.max(0, Math.min(100, score));

  if (score < 40) warnings.push('Overall health score is low — consider a maintenance session');
...
function renderDashboard(report: HealthReport, verbose: boolean) {
...
─── 📁 FILES ──────────────────────────────────────────────────
  ...
  if (files.orphans.length > 0) {
    ...
    if (files.orphans.length > 10) console.log(`  │  ... and ${files.orphans.length - 10} more`);
    console.log(`  └─`);
  }
...
─── 🔗 CROSS-REFERENCES ────────────────────────────────────────
  ...
  try {
    ...
    const criticalIssues = chunk.oversizedSections.filter((s: any) => 
      ['MEMORY.md', 'memory/OPERATING.md', 'memory/rules.md', 'memory/index.md', 'IDENTITY.md'].includes(s.file)
    );
    console.log(`
─── 📦 CHUNK HEALTH ────────────────────────────────────────────
...
─── 🔍 SEARCH DIAGNOSTICS ──────────────────────────────────────
  ...
  Total queries:   ${diagReport.total}`);
      if (diagReport.misses > 0) {
        const missedQueries = (diagReport.results || []).filter((r: any) => r.type === 'miss');
...
─── 📋 ALL TRACKED FILES ───────────────────────────────────────`);
    const sorted = Object.entries(report.weights.byType).length > 0 ? 
      Object.entries(JSON.parse(fs.readFileSync(MANIFEST_PATH, 'utf-8')).files as Record<string, ManifestFile>)
...
function runGate(report: HealthReport): { pass: boolean; failures: string[] } {
