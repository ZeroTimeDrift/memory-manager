...
interface RecallTest {
  query: string;
  expectedFile: string;       // Should appear in results (partial match)
  expectedContent: string;    // Key phrase in snippet (case-insensitive)
  importance: 'critical' | 'high' | 'medium' | 'low';
  description?: string;
}

interface FastBenchmarkResult {
  timestamp: string;
  score: number;
  tests: number;
  passed: number;
  memoryOnlyScore: number;
  allSourceScore: number;
  details: {
    query: string;
    importance: string;
    found: boolean;
    ...
    foundInAll: boolean;
    memoryRank: number | null;   // rank in memory-only results
    allRank: number | null;      // rank in all-source results
    topSource: string;           // source of top result in all-source
  }[];
  sourceNoise: {
    queriesWhereSessionBeatMemory: number;
    avgMemoryRank: number;
    avgAllRank: number;
  };
}
...
function searchFts(db: any, query: string, source?: string, limit: number = 10): {
  path: string; source: string; rank: number; text: string;
}[] {
...
function main(): void {
  ...
  // Get chunk counts
  const memChunks = db.prepare("SELECT COUNT(*) as c FROM chunks WHERE source='memory'").get() as any;
  const sesChunks = db.prepare("SELECT COUNT(*) as c FROM chunks WHERE source='sessions'").get() as any;
  console.log(`   📊 Chunks: ${memChunks.c} memory, ${sesChunks.c} sessions (${(sesChunks.c / memChunks.c).toFixed(1)}:1 ratio)`);
  console.log('');

  const result: FastBenchmarkResult = {
  ...
  for (const test of TESTS) {
    ...
    const memMatch = memResults.findIndex(r => 
    ...
    const allMatch = allResults.findIndex(r =>
      r.path.includes(test.expectedFile) ||
      r.text?.toLowerCase().includes(test.expectedContent.toLowerCase())
    );
  ...
  for (const d of result.details) {
    const w = importanceWeights[d.importance as keyof typeof importanceWeights] || 1;
    weightedTotal += w;
  ...
  // Save results
  let history: FastBenchmarkResult[] = [];
  try {
