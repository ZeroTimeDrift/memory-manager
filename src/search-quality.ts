...
interface SearchTest {
  ...
  query: string;
  /** Expected file path (partial match) — string or array of acceptable files */
  expectedFile: string | string[];
  /** Key content that should appear in the correct result */
  expectedContent: string;
  /** Importance weight */
  importance: 'critical' | 'high' | 'medium' | 'low';
  /** Optional tag for grouping analysis */
  category?: string;
}
...
interface QueryScore {
  ...
  importance: string;
  category: string;
  /** Rank of correct result (1-indexed), null if not found in top K */
  rank: number | null;
  /** Total results returned */
  resultsReturned: number;
  /** Score of the correct result */
  correctScore: number | null;
  /** Score of the top result (for gap analysis) */
  topScore: number;
  /** Gap between top score and correct score */
  scoreGap: number | null;
  /** The path of the correct result */
  correctPath: string | null;
  /** What outranked the correct answer (for debugging) */
  outrankedBy: string[];
}
...
interface QualityReport {
  ...
  precision: {
    p1: number;   // % of queries where correct answer is #1
    p3: number;   // % of queries where correct answer is top 3
    p5: number;   // % of queries where correct answer is top 5
    pAny: number; // % of queries where correct answer appears at all
  };
  
  /** Weighted precision (by importance) */
  weightedPrecision: {
    p1: number;
    p3: number;
    p5: number;
  ...
  /** Per-file precision: how easy is each file to find? */
  ...
  /** Weak spots — queries where answer exists but ranks poorly */
  weakSpots: {
    query: string;
    rank: number;
    scoreGap: number;
  ...
  /** Total failures — answer not found at all */
  failures: {
    query: string;
    importance: string;
    category: string;
  ...
  /** Composite score: 0.40 × wP@1 + 0.25 × wP@3 + 0.15 × wP@5 + 0.20 × wMRR (0-100) */
...
function searchMemory(query: string, maxResults: number = MAX_K): SearchResult[] {
  try {
...
function scoreQuery(test: SearchTest, results: SearchResult[]): QueryScore {
  let rank: number | null = null;
  let correctScore: number | null = null;
  let correctPath: string | null = null;
  const outrankedBy: string[] = [];
...
function computeReport(scores: QueryScore[]): QualityReport {
  const total = scores.length;
  const importanceWeights: Record<string, number> = { critical: 3, high: 2, medium: 1.5, low: 1 };

  // Basic precision@k
  const atK = (k: number) => scores.filter(s => s.rank !== null && s.rank <= k).length;
  ...
  // Per-importance breakdown
  const byImportance: QualityReport['byImportance'] = {};
  ...
  const byFile: QualityReport['byFile'] = {};
  ...
  return {
    ...
    compositeScore: Math.round(
      (0.40 * wp1 + 0.25 * wp3 + 0.15 * wp5 + 0.20 * weightedMrr) * 1000
    ) / 10,
  };
}
...
interface RegressionResult {
  ...
  absoluteFail: boolean;
  regressionFail: boolean;
  criticalFailures: string[];
  perQueryRegressions: { query: string; prevRank: number | null; currRank: number | null }[];
}

function checkRegression(report: QualityReport): RegressionResult {
  let history: QualityReport[] = [];
  try {
    history = JSON.parse(fs.readFileSync(RESULTS_PATH, 'utf-8'));
  } catch {}
  ...
  const perQueryRegressions: RegressionResult['perQueryRegressions'] = [];
  if (prev) {
...
function printRegression(result: RegressionResult): void {
  console.log('');
...
function showCompare(): void {
  ...
  for (const [name, p, c, unit] of metrics) {
    const delta = (c as number) - (p as number);
    const arrow = delta > 0 ? '📈' : delta < 0 ? '📉' : '➡️';
...
async function main(): Promise<void> {
  ...
  for (const test of tests) {
    ...
    if (done < tests.length) sleep(200);
  }
