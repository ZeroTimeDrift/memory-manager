...
interface ConceptEntry {
  ...
  concept: string;
  /** Category: person | tool | project | protocol | concept | place | org */
  category: ConceptCategory;
  /** Files where this concept appears, with frequency */
  files: Record<string, {
    count: number;
    sections: string[];  // Header context where it appears
    lastSeen: string;    // ISO date of file's last modification
  }>;
  /** Total mentions across all files */
  totalMentions: number;
  /** Related concepts (co-occur in same sections) */
  related: string[];
}

type ConceptCategory = 'person' | 'tool' | 'project' | 'protocol' | 'concept' | 'place' | 'org' | 'unknown';
...
const KNOWN_ENTITIES: Record<string, ConceptCategory> = {
  // People
  'hevar': 'person',
  'praneet': 'person',
  'amen': 'person',
...
function discoverMemoryFiles(): string[] {
  ...
  function walkDir(dir: string) {
    ...
    for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) {
        if (entry.name === 'archive') continue; // skip archives
        walkDir(fullPath);
  ...
  if (fs.existsSync(op) && !files.includes(op)) files.push(op);
...
function parseIntoSections(content: string): FileSection[] {
...
function categorize(term: string): ConceptCategory {
  const norm = normalize(term);
  if (KNOWN_ENTITIES[norm]) return KNOWN_ENTITIES[norm];
  
  // Heuristic categorization
  if (/^[A-Z][a-z]+$/.test(term)) return 'unknown'; // Could be a name
  if (term.includes('.') || term.includes('/')) return 'tool'; // Paths/domains
  return 'unknown';
}
...
function buildIndex(): ConceptIndex {
  ...
  const allConcepts: Record<string, ConceptEntry> = {};
  ...
  for (const filePath of files) {
    ...
    for (const section of sections) {
      ...
      const sectionConceptNames: string[] = [];
  ...
  const filtered: Record<string, ConceptEntry> = {};
  for (const [concept, entry] of Object.entries(allConcepts)) {
...
function loadIndex(): ConceptIndex {
  try {
...
function lookupConcept(term: string): void {
...
function printConceptEntry(entry: ConceptEntry): void {
...
function findRelated(term: string): void {
  const index = loadIndex();
  const norm = normalize(term);
  const entry = index.concepts[norm];
...
function suggestTags(text: string): void {
  const index = loadIndex();
  const concepts = extractConcepts(text);
  
  console.log('\n  🏷️  Suggested tags:\n');
  
  const tags: Array<{ tag: string; score: number; reason: string }> = [];
