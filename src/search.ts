...
class MemorySearch {
  ...
  private indexDirectory(dir: string, skipFiles: string[]): void {
...
async function main() {
  ...
  if (args.length === 0) {
    console.log(`Usage: npx ts-node src/search.ts "<query>" [limit]`);
    console.log(`       npx ts-node src/search.ts file "<filepath>" "<query>"`);
    process.exit(1);
  }
