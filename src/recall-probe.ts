...
interface ExtractedFact {
  ...
  section: string;
  fact: string;
  query: string;  // Natural-language query that should find this fact
  specificity: 'high' | 'medium' | 'low';  // How specific/unique is this fact
}

interface ProbeResult {
  fact: ExtractedFact;
  found: boolean;
  rank: number | null;  // Rank of correct file in results (null = not found)
  topResult: string;    // What file was actually returned first
  score: number;
}
 ...
 */
function extractFacts(filePath: string): ExtractedFact[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (entityProp) {
      ...
      if (entity.length < 4 || entityBlacklist.has(entity.toLowerCase())) continue;
      // Multi-word entities: reject if ALL words are short/common
      const entityWords = entity.split(/\s+/);
      if (entityWords.length > 1 && entityWords.every(w => w.length < 4 || entityBlacklist.has(w.toLowerCase()))) continue;
      facts.push({
        file: relPath,
    ...
    if (kvMatch) {
      ...
      // Skip generic keys
      if (['date', 'tags', 'mood', 'day', 'time', 'note'].includes(key.toLowerCase())) continue;
  ...
  return facts.filter(f => {
    if (!isUsefulQuery(f.query)) return false;
...
function generateQueryFromKV(key: string, value: string): string {
  const keyLower = key.toLowerCase();
  if (keyLower.includes('name') || keyLower.includes('who')) return `who is ${value.split(/[,(]/)[0].trim()}`;
  if (keyLower.includes('email')) return `what is the email for ${value.split('@')[0]}`;
  if (keyLower.includes('repo') || keyLower.includes('github')) return `where is the ${key.toLowerCase()} repository`;
  if (keyLower.includes('address') || keyLower.includes('wallet')) return `what is the wallet address for ${value.slice(0, 10)}`;
  if (keyLower.includes('work') || keyLower.includes('role')) return `what is ${key.toLowerCase()} about`;
  if (keyLower.includes('company') || keyLower.includes('org')) return `what company is ${value}`;
...
function extractTopicFromText(text: string): string {
  ...
  if (caps && caps.length > 0) return caps[0].toLowerCase();
...
function searchMemory(query: string, limit: number = 5): Array<{path: string, score: number, snippet: string}> {
  try {
    const { stmt } = getDb();
...
function runProbe(targetFile?: string): ProbeReport {
...
function showReport(): void {
  ...
  const report: ProbeReport = JSON.parse(fs.readFileSync(REPORT_PATH, 'utf-8'));
