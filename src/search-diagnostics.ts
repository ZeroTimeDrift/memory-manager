...
interface TokenAnalysis {
  ...
  fileOverlap: Map<string, { overlap: string[]; coverage: number }>;
}
...
interface Diagnosis {
  ...
  reason: string;
  confidence: number;
  strategies: string[];  // what strategies would help
}

interface Suggestion {
  type: 'add-keywords' | 'add-synonym' | 'create-proxy' | 'enrich-file' | 'add-concept';
  file: string;
  detail: string;
...
function searchBM25(query: string, limit: number = 10): BM25Hit[] {
  try {
...
function lookupConcepts(query: string): ConceptMatch[] {
  let index: any;
  try {
  ...
  for (const [entity, data] of Object.entries(index.concepts) as [string, any][]) {
...
function detectTemporal(query: string): { detected: boolean; dates: string[] } {
...
function analyzeTokens(query: string, targetFiles?: string[]): TokenAnalysis {
  ...
  const fileOverlap = new Map<string, { overlap: string[]; coverage: number }>();
...
function diagnose(
  ...
  bm25Results: BM25Hit[],
  conceptMatches: ConceptMatch[],
  temporal: { detected: boolean; dates: string[] },
  tokenAnalysis: TokenAnalysis,
  expectedFile?: string
): Diagnosis {
  ...
  if (!expectedFile) {
    ...
    if (hasResults) {
      ...
      if (tokenAnalysis.effectiveTokens.length <= 2) strategies.push('query-too-short');
      if (conceptMatches.length > 0) strategies.push('concept-index');
      if (temporal.detected) strategies.push('temporal');
  ...
  if (temporal.detected) strategies.push('temporal-routing');
...
function generateSuggestions(
  ...
  if (diagnosis.type === 'hit') return suggestions;
...
function runDiagnostic(query: string, expectedFile?: string, verbose: boolean = false): DiagnosticResult {
...
function printDiagnostic(result: DiagnosticResult, verbose: boolean = false) {
  const icon = result.diagnosis.type === 'hit' ? '✅' : result.diagnosis.type === 'partial' ? '🟡' : '🔴';
  
  console.log(`\n${icon} Query: "${result.query}"`);
  if (result.expectedFile) console.log(`   Expected: ${result.expectedFile}`);
...
function main() {
  ...
  if (batch) {
    ...
    for (const spot of KNOWN_BLIND_SPOTS) {
      ...
      if (result.diagnosis.type === 'hit') hits++;
      else if (result.diagnosis.type === 'partial') partials++;
      else misses++;
    }
